...
\* operators
BinTypes == {"trash", "recycle"} \* constant op (without args)
SetsOfFour(set) == set \X set \X set \X set \* with arg
Items == [type: BinTypes, size: 1..6] \* set of structs
...
variables
 ...
 \* If you want to define an operator using the variables, you can place it in a define block:
define
    NoBinOverflow ==
...
\* BEGIN TRANSLATION (chksum(pcal) = "4a8c98aa" /\ chksum(tla) = "4b8b6e03")
